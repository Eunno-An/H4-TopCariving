...
class CarOptionRepositoryTest {
	...
	@Test
	void findByArchivingIdAndCategoryDetailEmpty() {
		// given
		Long archivingId = 1L;
		CategoryDetail categoryDetail = CategoryDetail.H_GENUINE_ACCESSORIES;

		// when
		Optional<CarOption> carOption = carOptionRepository.findByArchivingIdAndCategoryDetail(archivingId, categoryDetail);

		// then
		Assertions.assertThat(carOption).isEmpty();
	}

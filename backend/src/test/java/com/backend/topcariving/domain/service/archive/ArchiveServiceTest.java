...
@SpringBootTest
@Transactional
class ArchiveServiceTest extends TestSupport {
	...
	class Feed {
		...
		@Test
		void 피드에서_저장한_차량이_없다면_빈_배열이_반환되어야_한다() {
			// given
			Long userId = 10L;
			Integer pageNumber = 1;
			Integer pageSize = 1;
			...
			// when
			List<ArchiveFeedDTO> archiveFeedCars = archiveService.getFeedCars(userId, pageNumber, pageSize);

			// then
			Assertions.assertThat(archiveFeedCars).hasSize(0);
		}
	}
	...
	class MyCariving {
		...
		@Test
		void 내가_만든_차량이_없다면_빈_배열이_반환되어야_한다() {
			// given
			Long userId = 10L;
			Integer pageNumber = 1;
			Integer pageSize = 1;
			...
			// then
			Assertions.assertThat(createdCars).hasSize(0);
		}
	}

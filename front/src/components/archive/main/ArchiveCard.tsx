    ...
    <Wrapper direction="column" justify="flex-start" align="flex-start">
      <Flex height="auto" justify="space-between" margin="0 0 5px 0">
        <Text typo="Heading3_Bold">
          {`펠리세이드 ${archiveInfo.carArchiveResult.모델[0]}`}
        </Text>
        <BrownTag>{`${getDate(new Date(archiveInfo.dayTime))}에 ${
          archiveInfo.type
        }했어요`}</BrownTag>
      </Flex>
      <Text typo="Body3_Regular">{`${archiveInfo.carArchiveResult.엔진[0]} / ${archiveInfo.carArchiveResult.구동방식[0]} / ${archiveInfo.carArchiveResult.바디타입[0]}`}</Text>
      ...
      <Flex height="auto" gap={7} justify="flex-start" align="flex-start">
        ...
        <Flex
          ...
          `}
        >
          {archiveInfo.carArchiveResult['상세 품목'].map((optionName, idx) => {
